...
export function alertMessage(opts: {
  ...
  return [
    ...
    `• ${fmtPct(opts.score.realizedPnLPercent)} realized PnL (${opts.score.sampleSize} closed trades)`,
    `• ${Math.round(opts.score.winRate * 100)}% overall win rate`,
    assetStats,
    `• Last 5: ${lastFive}`,
  ].join("\n");
}
...
export function scoreBreakdown(score: CredibilityScore, address: string): string {
  const lines = [
    ...
    `• Overall win rate: ${Math.round(score.winRate * 100)}% (${score.sampleSize} closed trades)`,
